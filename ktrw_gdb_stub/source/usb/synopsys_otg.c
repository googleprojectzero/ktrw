...
static void ep0_begin_data_in_stage(const void *data, uint32_t size, void (*callback)(void));
static void ep0_begin_data_out_stage(bool (*callback)(const void *data, uint32_t size));
...
ep_in_send_done(struct endpoint_state *ep) {
	...
	if (ep->transferred == ep->transfer_size) {
		...
		bool need_zlp = (ep->transfer_size > 0
				&& ep->transfer_size % ep->max_packet_size == 0);
		if (!need_zlp) {
...
//
// The data buffer must remain valid until the transfer is complete.
static void
...
ep_out_recv_data_done(struct endpoint_state *ep) {
	...
	if (ep->type == 0) {
		...
		bool whole_number_of_packets = (data_received > 0
				&& data_received % ep->max_packet_size == 0);
		if (partial_packet && whole_number_of_packets) {
			...
			// see packets_received == expected_packets (no ZLP) or packets_received ==
			...
			if (packets_received == expected_packets + 1) {
				...
				if (packets_received != expected_packets) {
					USB_DEBUG(USB_DEBUG_FATAL, "EP%u OUT: Unexpected number "
							"of packets received: %u != %u",
							ep->n, packets_received, expected_packets);
					USB_DEBUG_ABORT();
				}
...
ep0_out_interrupt() {
	...
	if ((doepint & 0x8) && ep0.setup_packet_pending) {
		...
		// transfer in the default DMA buffer.
		...
		if (done) {
			// ep_out_recv_data_done() has reset the receive state, so the next call
			// to ep_out_recv() expects a setup packet. But before we start receiving
			...
			// The DATA OUT transaction is not done. The call to ep_out_recv() below
...
usb_start() {
	...
	reg_write(rDIEPINT(0), 0x1f);
	reg_write(rDOEPINT(0), 0xf);
	reg_write(rGINTMSK, 0x1000);	// Only listen on EP 0 OUT at the start.
	reg_and(rDCTL, ~0x2);
	// Perform initial activations to set up endpoint state.
	ep_out_activate(&ep0_out, 0, 0, EP0_MAX_PACKET_SIZE);
	ep_in_activate(&ep0_in, 0, 0, EP0_MAX_PACKET_SIZE, 0);
}
...
usb_process() {
	...
	USB_DEBUG_ABORT_ON_ITERATION(8000000);
}
